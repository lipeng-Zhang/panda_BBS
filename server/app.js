...
//关于candidatePost
app
  .route('/candidatePosts/:categoryId')
  ...
  .get(async (req, res) => {
    let id = req.params.categoryId
    if (id != 0) {
      let posts = await db.all(sqlMap.candidatePosts.data, id)
      res.json(posts)
    } else {
      let posts = await db.all('select * from candidatePosts order by postId desc')
      res.json(posts)
    }
  })
  .post(async (req, res) => {
...
//根据postId删除candidate post
app.post('/candidateDelete', async (req, res) => {
  let postId = req.query.postId
  let post = await db.all(sqlMap.candidatePosts.delete, Number(postId))
  res.json(post)
})
...
app.post('/Delete', async (req, res) => {
  let postId = req.query.postId
  let post = await db.all(sqlMap.posts.delete, Number(postId))
  res.json(post)
})
